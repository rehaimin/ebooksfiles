<div class="card" id="filesListCard">
  <div class="card-header">Liste des fichiers</div>
  <div class="card-body">
    <div class="row justify-content-between align-items-center mb-3">
      <div class="col-md-2">
        <a href="{{ route('files.create') }}" class="btn btn-primary my-3" id="addFile">Ajouter un Fichier</a>
      </div>
      <form method="GET" action="{{ route('files.index') }}" accept-charset="UTF-8"
        class="form-inline my-2 my-lg-0 float-right col-md-6" role="search" id="searchForm">
        @csrf
        <div class="input-group">
          <input type="search" class="form-control" name="search" placeholder="Recherche..."
            value="{{ request('search') }}" id="searchTerm">
      ...
      <table class="table-responsive table table-striped table-light">
        <thead>
          <tr>
            ...
            <th style="width: 15%;" class="actionHeader">Action</th>
          </tr>
        </thead>
        <tbody>
          @foreach ($files as $file)
            <tr>
              ...
              <td style="width: 15%; text-align:center;" class="actionButtons">
                {{-- Delete File --}}
                <button type="button" class="btn" data-bs-toggle="modal" data-bs-target="#modal{{ $file->id }}"
                  style="border:none;">
